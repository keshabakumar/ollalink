...
cp /tmp/auth-form.tsx /opt/ollalink/frontend/dashboard/src/components/auth-form.tsx
...
systemd-run --unit=cvxlogs --working-directory=/opt/ollalink/backend/convex /bin/bash -lc "/usr/bin/node /opt/ollalink/node_modules/convex/bin/main.js logs > /tmp/cvxlogs.txt 2>&1" >/dev/null 2>&1
