...
cp /tmp/backend.ts  /opt/ollalink/packages/backend/convex/backend.ts
...
mkdir -p /opt/ollalink/apps/app/src/lib
cp /tmp/backend.config.ts /opt/ollalink/apps/app/src/lib/backend.config.ts
cp /tmp/backend-client.ts /opt/ollalink/apps/app/src/lib/backend-client.ts
mkdir -p /opt/ollalink/backend-echo
cp /tmp/server.mjs  /opt/ollalink/backend-echo/server.mjs
echo ok

echo "=== secrets + echo backend env ==="
if [ ! -f /opt/ollalink/backend-echo/.env ]; then
  SERVICE_KEY="$(openssl rand -hex 24)"
  WEBHOOK_SECRET="$(openssl rand -hex 24)"
  cat > /opt/ollalink/backend-echo/.env <<EOF
PORT=4000
...
[Service]
Type=simple
WorkingDirectory=/opt/ollalink/backend-echo
EnvironmentFile=/opt/ollalink/backend-echo/.env
ExecStart=/usr/bin/node /opt/ollalink/backend-echo/server.mjs
Restart=always
RestartSec=5
