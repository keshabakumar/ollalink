...
```
Browser ─▶ Next.js dashboard (frontend/dashboard)
              │  reactive queries/mutations (live)
              ▼
        CONVEX control-plane (backend/convex)
        auth(OTP) · users · orgs/RBAC · jobs · files
        usage · api keys · audit · billing · JWKS · webhooks
              │ proxy (service key)        ▲ webhook (results/events)
              ▼                            │
        YOUR REAL BACKEND (swappable, any language)  ── reference impl: backend/reference-api/
```
...
`backend/reference-api/server.mjs` is a ~120-line reference backend implementing all three (JWKS verify in ~10 lines). Swap it for your real backend by changing `BACKEND_BASE_URL` + `NEXT_PUBLIC_BACKEND_URL`.
